...
class MessagePackUnkeyedDecodingContainer : UnkeyedDecodingContainer {
  ...
  func decodeNil() throws -> Bool {
    guard !self.isAtEnd else {
      throw DecodingError.valueNotFound(Any?.self, DecodingError.Context(codingPath: self.decoder.codingPath + [MessagePackKey(index: self.currentIndex)], debugDescription: "Unkeyed container is at end."))
    }
  ...
  func decode(_ type: Bool.Type) throws -> Bool {
    ...
    self.currentIndex += 1
    return try decoder.unbox(self.container[self.currentIndex], as: type)
  }
  ...
  func decode<T>(_ type: T.Type) throws -> T where T : Decodable {
    try expectNotAtEnd(type)
    self.currentIndex += 1
    guard let value = try decoder.unbox(self.container[self.currentIndex], as: type) else {
